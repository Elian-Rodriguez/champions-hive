export default function LeagueMetrics2({ data }: { data?: any }) {
  const standings = data?.standingsFlat ?? [];

  // FALLBACK: no real standings data → render the ORIGINAL hardcoded markup unchanged.
  if (!standings.length) {
    return (
  ...
  // REAL DATA PATH
  const tournamentName = data?.tournament?.name ?? "Pro League Standings";
  const stats = data?.stats ?? null;
  const teamIcons = ["bolt", "diamond", "waves", "local_fire_department"];
  const teamIconColors = ["text-secondary", "text-tertiary", "text-blue-400", "text-orange-400"];
  const teamPhoto =
    "https://lh3.googleusercontent.com/aida/ADBb0ujMxExzo8Z3tDIBmPwgn5rdVFvapuuaGh-cktOlKsvDQnSa3C99sGooYLOlYDFGrOkE5rnwKz8j4O4xgGGw8I7ZKX0S3C9WEg5FYWVSuT5wN2Goz32-PeZrg1H-C3fY3ddj4xXVY0i6NWpJ06SL9QzblMF7NFj_Qkb1yEXfk1G9pjlRJPqcImnej2MUJeS_V1CqreL_q3CucM37JO7QNxziXImlEqxHM8ZQPTYjH40lSDBDTFa6R9lGWqvn";
    ...
    <div className="bg-background text-on-surface font-body-md overflow-x-hidden">
      ...
      <main className="ml-64 mt-16 p-8 min-h-screen">
        ...
        <div className="flex justify-between items-end mb-8">
          <div>
            <nav className="flex items-center gap-2 text-xs text-slate-500 font-label-sm mb-2">
              <span>DASHBOARD</span>
              <span className="material-symbols-outlined text-[10px]">chevron_right</span>
              <span className="text-secondary">{tournamentName.toUpperCase()}</span>
            </nav>
            <h2 className="font-headline-lg text-display-xl text-on-surface tracking-tighter">League Metrics</h2>
          </div>
          <div className="flex gap-3">
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-8 bg-surface-container-low rounded-xl border border-slate-800 p-6">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-headline-md text-on-surface">{tournamentName}</h3>
              <div className="flex gap-2">
                <span className="bg-secondary/10 text-secondary text-[10px] font-bold px-2 py-1 rounded">LIVE UPDATING</span>
              </div>
            </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-slate-800 text-slate-500 font-label-sm">
                    ...
                    <th className="pb-4 px-2 text-center">L</th>
                    <th className="pb-4 px-4 text-center">PTS</th>
                    <th className="pb-4 px-2">DIFF</th>
                  </tr>
                </thead>
                <tbody className="font-stats-numeric">
                  {standings.map((row: any, i: number) => {
                    ...
                    const iconColor = teamIconColors[i % teamIconColors.length];
                    const diff = row?.diff ?? 0;
                    return (
                      <tr
                        key={row?.team_name ?? position ?? i}
                        className={
                          "group hover:bg-slate-800/30 transition-colors" +
                          (i > 0 ? " border-t border-slate-800/50" : "")
                        }
                      >
                        <td className={"py-4 px-2 font-bold" + (i === 0 ? " text-secondary" : " text-on-surface")}>
                          {String(position).padStart(2, "0")}
                        </td>
                        <td className="py-4 px-2">
                          <div className="w-10 h-10 rounded-full border border-slate-700 overflow-hidden bg-slate-800">
                            <img alt={(row?.team_name ?? "Team") + " Member"} className="w-full h-full object-cover" src={teamPhoto} />
                          </div>
                        </td>
                        <td className="py-4 px-2">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-headline-md text-sm">{row?.team_name ?? "—"}</span>
                          </div>
                        </td>
                        <td className="py-4 px-2 text-center text-slate-400">{row?.matches_played ?? 0}</td>
                        <td className="py-4 px-2 text-center">{row?.wins ?? 0}</td>
                        <td className="py-4 px-2 text-center text-slate-400">{row?.draws ?? 0}</td>
                        <td className="py-4 px-2 text-center text-slate-400">{row?.losses ?? 0}</td>
                        <td className="py-4 px-4 text-center font-black text-on-surface text-lg">{row?.league_points ?? 0}</td>
                        <td className="py-4 px-2 text-center text-slate-400">{diff > 0 ? "+" + diff : diff}</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="mt-6 pt-6 border-t border-slate-800 flex justify-center">
              <button className="text-secondary font-label-sm hover:underline">VIEW FULL STANDINGS ({standings.length} TEAMS)</button>
            </div>
          </div>
          {/* Performance Curve (Visual Data) */}
          <div className="col-span-4 space-y-6">
            ...
            <div className="bg-surface-container-high rounded-xl border-l-4 border-secondary p-6 shadow-xl">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <p className="text-[10px] text-secondary font-bold uppercase tracking-widest mb-1">STREAK ALERT</p>
                  <h4 className="font-headline-md text-on-surface">{standings[0]?.team_name ?? "—"}</h4>
                </div>
                <span className="material-symbols-outlined text-secondary text-3xl">trending_up</span>
              </div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-xs font-bold text-on-surface">{standings[0]?.wins ?? 0} MATCH WIN STREAK</span>
              </div>
              <div className="text-[10px] text-slate-400 grid grid-cols-2 gap-2">
                <div className="bg-slate-950/50 p-2 rounded">
                  <p>POINTS FOR</p>
                  <p className="text-on-surface font-bold text-sm">{standings[0]?.points_scored ?? 0}</p>
                </div>
                <div className="bg-slate-950/50 p-2 rounded">
                  <p>POINTS AGAINST</p>
                  <p className="text-on-surface font-bold text-sm">{standings[0]?.points_conceded ?? 0}</p>
                </div>
              </div>
            </div>
          </div>
          {/* League Summary (KPI tiles from data.stats) */}
          <div className="col-span-12 grid grid-cols-4 gap-6">
            <div className="col-span-3 bg-slate-950 rounded-xl border border-slate-800 overflow-hidden relative">
              <div className="absolute inset-0 bg-gradient-to-r from-slate-950 via-slate-950/40 to-transparent z-10"></div>
              <div className="absolute inset-y-0 right-0 w-1/2 overflow-hidden">
              ...
              <div className="relative z-20 p-8 h-full flex flex-col justify-center max-w-lg">
                <span className="text-secondary font-bold text-xs tracking-[0.3em] mb-2 uppercase">League Leader</span>
                <h2 className="text-display-xl font-display-xl text-on-surface mb-2">{standings[0]?.team_name ?? "—"}</h2>
                <div className="flex items-center gap-6 mb-6">
                  <div className="flex flex-col">
                    <span className="text-slate-500 text-[10px] uppercase font-bold">League Points</span>
                    <span className="text-3xl font-stats-numeric text-secondary">{standings[0]?.league_points ?? 0}</span>
                  </div>
                  <div className="w-[1px] h-10 bg-slate-800"></div>
                  <div className="flex flex-col">
                    <span className="text-slate-500 text-[10px] uppercase font-bold">Points For</span>
                    <span className="text-3xl font-stats-numeric text-on-surface">{standings[0]?.points_scored ?? 0}</span>
                  </div>
                  <div className="w-[1px] h-10 bg-slate-800"></div>
                  <div className="flex flex-col">
                    <span className="text-slate-500 text-[10px] uppercase font-bold">Diff</span>
                    <span className="text-3xl font-stats-numeric text-on-surface">{(standings[0]?.diff ?? 0) > 0 ? "+" + (standings[0]?.diff ?? 0) : (standings[0]?.diff ?? 0)}</span>
                  </div>
                </div>
                <button className="bg-secondary text-slate-950 w-fit px-6 py-3 rounded-lg font-bold text-sm hover:translate-x-1 transition-transform flex items-center gap-2">
                  VIEW STANDINGS <span className="material-symbols-outlined text-sm">arrow_forward</span>
                </button>
              </div>
            </div>
            {/* League Stat Tiles */}
            <div className="col-span-1 flex flex-col gap-4">
              <div className="bg-surface-container rounded-xl border border-slate-800 p-4 flex items-center gap-4 hover:border-slate-600 transition-all cursor-pointer">
                <div className="w-12 h-12 rounded bg-slate-800 flex-shrink-0 flex items-center justify-center text-secondary">
                  <span className="material-symbols-outlined" style={{ fontVariationSettings: "'FILL' 1" }}>groups</span>
                </div>
                <div>
                  <p className="font-headline-md text-sm text-on-surface">{stats?.teams ?? standings.length}</p>
              ...
              <div className="bg-surface-container rounded-xl border border-slate-800 p-4 flex items-center gap-4 hover:border-slate-600 transition-all cursor-pointer">
                ...
                <div>
                  <p className="font-headline-md text-sm text-on-surface">{stats?.finished_matches ?? 0}<span className="text-slate-500"> / {stats?.matches ?? 0}</span></p>
              ...
              <div className="bg-surface-container rounded-xl border border-slate-800 p-4 flex items-center gap-4 hover:border-slate-600 transition-all cursor-pointer">
                ...
                <div>
                  <p className="font-headline-md text-sm text-on-surface">{stats?.stages ?? 0}</p>
