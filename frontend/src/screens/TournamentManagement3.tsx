export default function TournamentManagement3({ data }: { data?: any }) {
  // FALLBACK: render the original hardcoded markup when there's no real tournament.
  if (!data?.tournament) {
  return (
    <div className="bg-background text-on-background font-body-md custom-scrollbar">
      ...
      <main className="ml-64 mt-16 p-8 min-h-screen">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-surface-container-low rounded-xl overflow-hidden border border-secondary/30 flex flex-col group hover:border-secondary transition-all duration-300" id="Copa-de-Titanes-2024-card">
          ...
          <div className="bg-surface-container-low rounded-xl overflow-hidden border border-slate-800 flex flex-col group hover:border-slate-700 transition-all duration-300" id="Summer-Dunk-Classic-card">
            <div className="relative h-48">
  ...
  const rounds: any[] = Array.isArray(data?.bracket?.rounds) ? data.bracket.rounds : [];
        ...
        <div className="flex justify-between items-end mb-8">
          <div>
            ...
            <p className="font-body-md text-on-surface-variant mt-1">
              {[tournament?.sport_type, tournament?.category, tournament?.status].filter(Boolean).join(' • ') || 'Oversee active leagues, registrations, and historical archives.'}
            </p>
          </div>
          <button className="bg-secondary text-on-secondary px-6 py-2.5 rounded-lg font-bold flex items-center gap-2 hover:brightness-110 active:scale-95 transition-all shadow-lg shadow-secondary/10">
            <span className="material-symbols-outlined" data-icon="add_circle">add_circle</span>
        ...
        {hasBracket ? (
          ...
          <div className="flex gap-8 overflow-x-auto pb-4">
            {rounds.map((round: any, rIdx: number) => (
              <div key={round?.round ?? rIdx} className="flex flex-col gap-6 min-w-[280px]">
                <h3 className="font-headline-md text-sm uppercase tracking-wider text-on-surface">{round?.name ?? `Round ${round?.round ?? rIdx + 1}`}</h3>
                {(Array.isArray(round?.matches) ? round.matches : []).map((m: any, mIdx: number) => (
                  <div key={mIdx} className="bg-surface-container-low rounded-xl border border-slate-800 p-4 hover:border-secondary transition-all duration-300">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-bold text-on-surface">{m?.home_team_name ?? 'TBD'}</span>
                      <span className="font-stats-numeric text-stats-numeric text-secondary">{m?.home_score ?? '-'}</span>
                    </div>
                    <div className="w-full h-px bg-slate-800 my-2"></div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-bold text-on-surface">{m?.away_team_name ?? 'TBD'}</span>
                      <span className="font-stats-numeric text-stats-numeric text-secondary">{m?.away_score ?? '-'}</span>
                    </div>
                    {m?.status ? (
                      <span className="mt-3 inline-block bg-slate-900/80 backdrop-blur px-2 py-0.5 rounded text-[10px] font-bold text-secondary border border-secondary/20 uppercase">{m.status}</span>
                    ) : null}
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {matches.map((m: any, mIdx: number) => (
              <div key={m?.id ?? mIdx} className="bg-surface-container-low rounded-xl overflow-hidden border border-slate-800 flex flex-col group hover:border-secondary transition-all duration-300">
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex justify-between items-center mb-4">
                    <div>
                      <p className="text-label-sm font-label-sm text-slate-500 mb-1">HOME</p>
                      <p className="text-sm font-bold text-on-surface">{m?.home_team_name ?? 'TBD'}</p>
                    </div>
                    <p className="font-stats-numeric text-stats-numeric text-on-surface">{m?.home_score ?? '-'} : {m?.away_score ?? '-'}</p>
                    <div className="text-right">
                      <p className="text-label-sm font-label-sm text-slate-500 mb-1">AWAY</p>
                      <p className="text-sm font-bold text-on-surface">{m?.away_team_name ?? 'TBD'}</p>
                    </div>
                  </div>
                  {m?.status ? (
                    <span className="mt-auto inline-block bg-slate-900/80 backdrop-blur px-2 py-0.5 rounded text-[10px] font-bold text-secondary border border-secondary/20 uppercase">{m.status}</span>
                  ) : null}
                </div>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {teams.map((team: any, tIdx: number) => (
              <div key={team?.id ?? tIdx} className="bg-surface-container-low rounded-xl overflow-hidden border border-slate-800 flex flex-col group hover:border-secondary transition-all duration-300">
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex justify-between mb-4">
                    <div>
                      <p className="text-label-sm font-label-sm text-slate-500 mb-1">TEAM</p>
                      <p className="font-stats-numeric text-stats-numeric text-on-surface">{team?.name ?? 'Unnamed Team'}</p>
                    </div>
                    {team?.group_name ? (
                      <div className="text-right">
                        <p className="text-label-sm font-label-sm text-slate-500 mb-1">GROUP</p>
                        <p className="font-stats-numeric text-stats-numeric text-secondary">{team.group_name}</p>
                      </div>
                    ) : null}
                  </div>
                  {team?.status ? (
                    <span className="mt-auto inline-block bg-slate-900/80 backdrop-blur px-2 py-0.5 rounded text-[10px] font-bold text-secondary border border-secondary/20 uppercase">{team.status}</span>
                  ) : null}
                </div>
              </div>
            ))}
