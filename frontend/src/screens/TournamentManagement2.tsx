export default function TournamentManagement2({ data }: { data?: any }) {
  if (!data?.tournament) {
  return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-surface-container-low rounded-xl overflow-hidden border border-secondary/30 flex flex-col group hover:border-secondary transition-all duration-300" id="Copa-de-Titanes-2024-card">
            ...
            <div className="p-6 flex-1 flex flex-col">
              {/* Team Photo Preview */}
              <div className="mb-6 flex items-center justify-between bg-surface-container-high/50 p-3 rounded-lg border border-outline-variant/10">
                <div className="flex items-center gap-3">
                  <div className="relative w-12 h-12 rounded-md overflow-hidden border border-secondary/20 bg-slate-800">
                    <img alt="Active Team Photo Preview" className="w-full h-full object-cover" src="https://lh3.googleusercontent.com/aida/ADBb0uiH2R2tC5Zp9mYvP9bQnL7JkY2xKz-Q2fWv-uF_vXj_Z4aR0pQ-wB7D1LzU2m_rW7O0o_X-R8s_B6W_Z9mF8L7O0" />
                  </div>
                  <div>
                    <p className="text-[10px] font-bold text-secondary uppercase tracking-wider">Official Team Photo</p>
                    <p className="text-xs text-on-surface-variant">Uploaded by Admin</p>
                  </div>
                </div>
          ...
          <div className="bg-surface-container-low rounded-xl overflow-hidden border border-slate-800 flex flex-col group hover:border-slate-700 transition-all duration-300" id="Summer-Dunk-Classic-card">
            ...
            <div className="p-6 flex-1 flex flex-col">
              {/* Upload Action */}
              <button className="mb-6 flex items-center gap-3 w-full p-3 rounded-lg border-2 border-dashed border-outline-variant/30 hover:border-secondary/50 hover:bg-secondary/5 transition-all group/upload">
                <div className="w-10 h-10 rounded-full bg-surface-container-high flex items-center justify-center text-on-surface-variant group-hover/upload:text-secondary group-hover/upload:bg-secondary/10 transition-colors">
                  <span className="material-symbols-outlined" data-icon="add_a_photo">add_a_photo</span>
                </div>
                <div className="text-left">
                  <p className="text-xs font-bold text-on-surface">Upload Team Photo</p>
                  <p className="text-[10px] text-slate-500 italic">Optional Feature</p>
                </div>
              </button>
              <div className="flex justify-between mb-4">
                <div>
    ...
    <div className="bg-background text-on-background font-body-md custom-scrollbar">
      ...
      <main className="ml-64 mt-16 p-8 min-h-screen">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div key={team?.id ?? i} className="bg-surface-container-low rounded-xl overflow-hidden border border-secondary/30 flex flex-col group hover:border-secondary transition-all duration-300" id={`${team?.name ?? 'team'}-card`}>
            <div className="relative h-48">
              <img className="w-full h-full object-cover grayscale-[0.5] group-hover:grayscale-0 transition-all duration-500" data-alt="A dynamic, low-angle shot of a high-end soccer stadium at night under bright floodlights. A professional-grade football sits on a lush green turf in the foreground, with the stands blurred in the distance. The lighting is dramatic and cinematic, featuring neon green highlights reflecting off the wet grass. The atmosphere is intense, ready for a major championship final." src="https://lh3.googleusercontent.com/aida/ADBb0ujMxExzo8Z3tDIBmPwgn5rdVFvapuuaGh-cktOlKsvDQnSa3C99sGooYLOlYDFGrOkE5rnwKz8j4O4xgGGw8I7ZKX0S3C9WEg5FYWVSuT5wN2Goz32-PeZrg1H-C3fY3ddj4xXVY0i6NWpJ06SL9QzblMF7NFj_Qkb1yEXfk1G9pjlRJPqcImnej2MUJeS_V1CqreL_q3CucM37JO7QNxziXImlEqxHM8ZQPTYjH40lSDBDTFa6R9lGWqvn" />
              <div className="absolute top-4 left-4 bg-tertiary-container/80 backdrop-blur-md px-3 py-1 rounded-full flex items-center gap-2 border border-tertiary/40">
                <span className="w-2 h-2 rounded-full bg-tertiary-fixed-dim live-pulse"></span>
                <span className="text-[10px] font-bold text-tertiary-fixed tracking-widest uppercase">{team?.status ?? 'Active'}</span>
              </div>
              <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-slate-950 to-transparent">
                <span className="bg-slate-900/80 backdrop-blur px-2 py-0.5 rounded text-[10px] font-bold text-secondary border border-secondary/20 mb-2 inline-block">{team?.group_name ?? 'GROUP'}</span>
                <h3 className="font-headline-md text-headline-md text-white">{team?.name ?? 'Team'}</h3>
              </div>
            </div>
            <div className="p-6 flex-1 flex flex-col">
              {/* Team Photo Preview */}
              ...
              <div className="flex justify-between mb-4">
                <div>
                  <p className="text-label-sm font-label-sm text-slate-500 mb-1">MATCHES</p>
                  <p className="font-stats-numeric text-stats-numeric text-on-surface">{stats?.matches ?? 0} <span className="text-xs font-normal text-slate-500">total</span></p>
                </div>
                <div className="text-right">
                  <p className="text-label-sm font-label-sm text-slate-500 mb-1">TEAMS</p>
                  <p className="font-stats-numeric text-stats-numeric text-on-surface">{stats?.teams ?? teams.length}</p>
                </div>
              </div>
              <div className="w-full bg-slate-800 h-1.5 rounded-full mb-6 overflow-hidden">
                <div className="bg-secondary h-full rounded-full w-[68%]" style={{ boxShadow: '0 0 10px rgba(74, 225, 118, 0.5)' }}></div>
