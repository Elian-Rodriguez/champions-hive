export default function TournamentManagement1({ data }: { data?: any }) {
  // FALLBACK: no real tournament -> render the ORIGINAL hardcoded markup unchanged.
  if (!data?.tournament) {
  return (
  ...
  const finishedMatches = stats?.finished_matches ?? 0;
  const totalMatches = stats?.matches ?? matches.length;
  ...
  return (
    <div className="bg-background text-on-background font-body-md custom-scrollbar">
      ...
      <main className="ml-64 mt-16 p-8 min-h-screen">
        ...
        <div className="flex justify-between items-end mb-8">
          <div>
            <h2 className="font-headline-lg text-headline-lg text-on-surface">{tournament?.name ?? "Tournament Management"}</h2>
            <p className="font-body-md text-on-surface-variant mt-1">
              {[tournament?.sport_type, tournament?.status, tournament?.category].filter(Boolean).join(" • ") || "Oversee active leagues, registrations, and historical archives."}
            </p>
          </div>
          <button className="bg-secondary text-on-secondary px-6 py-2.5 rounded-lg font-bold flex items-center gap-2 hover:brightness-110 active:scale-95 transition-all shadow-lg shadow-secondary/10">
            <span className="material-symbols-outlined" data-icon="add_circle">add_circle</span>
        ...
        <div className="flex flex-wrap items-center gap-4 mb-8 bg-surface-container-low p-4 rounded-xl border border-outline-variant/30">
          <div className="flex items-center gap-2 bg-surface-container-high px-3 py-1.5 rounded-lg border border-outline-variant/20">
            <span className="text-label-sm font-label-sm text-on-surface-variant">Teams:</span>
            <span className="text-on-surface font-bold text-sm">{stats?.teams ?? teams.length}</span>
          </div>
          <div className="flex items-center gap-2 bg-surface-container-high px-3 py-1.5 rounded-lg border border-outline-variant/20">
            <span className="text-label-sm font-label-sm text-on-surface-variant">Stages:</span>
            <span className="text-on-surface font-bold text-sm">{stats?.stages ?? stages.length}</span>
          </div>
          <div className="flex items-center gap-2 bg-surface-container-high px-3 py-1.5 rounded-lg border border-outline-variant/20">
            <span className="text-label-sm font-label-sm text-on-surface-variant">Matches:</span>
            <span className="text-on-surface font-bold text-sm">{finishedMatches}/{totalMatches}</span>
          </div>
          <div className="ml-auto flex items-center gap-2">
            <span className="text-label-sm font-label-sm text-slate-500">View:</span>
            <div className="flex bg-slate-900 rounded-lg p-1">
        ...
        {/* Tournament Cards Grid -> mapped over data.matches */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {matches.map((match: any, idx: number) => {
            const isFinished = String(match?.status ?? "").toLowerCase().includes("finish");
            const isLive = String(match?.status ?? "").toLowerCase().includes("live") || String(match?.status ?? "").toLowerCase().includes("progress");
            return (
              <div key={match?.id ?? idx} className="bg-surface-container-low rounded-xl overflow-hidden border border-secondary/30 flex flex-col group hover:border-secondary transition-all duration-300">
                <div className="relative h-48">
                  <img className="w-full h-full object-cover grayscale-[0.5] group-hover:grayscale-0 transition-all duration-500" data-alt="A dynamic, low-angle shot of a high-end soccer stadium at night under bright floodlights. A professional-grade football sits on a lush green turf in the foreground, with the stands blurred in the distance. The lighting is dramatic and cinematic, featuring neon green highlights reflecting off the wet grass. The atmosphere is intense, ready for a major championship final." src="https://lh3.googleusercontent.com/aida/ADBb0ujMxExzo8Z3tDIBmPwgn5rdVFvapuuaGh-cktOlKsvDQnSa3C99sGooYLOlYDFGrOkE5rnwKz8j4O4xgGGw8I7ZKX0S3C9WEg5FYWVSuT5wN2Goz32-PeZrg1H-C3fY3ddj4xXVY0i6NWpJ06SL9QzblMF7NFj_Qkb1yEXfk1G9pjlRJPqcImnej2MUJeS_V1CqreL_q3CucM37JO7QNxziXImlEqxHM8ZQPTYjH40lSDBDTFa6R9lGWqvn" />
                  <div className="absolute top-4 left-4 bg-tertiary-container/80 backdrop-blur-md px-3 py-1 rounded-full flex items-center gap-2 border border-tertiary/40">
                    {isLive ? <span className="w-2 h-2 rounded-full bg-tertiary-fixed-dim live-pulse"></span> : null}
                    <span className="text-[10px] font-bold text-tertiary-fixed tracking-widest uppercase">{match?.status ?? "Scheduled"}</span>
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-slate-950 to-transparent">
                    <span className="bg-slate-900/80 backdrop-blur px-2 py-0.5 rounded text-[10px] font-bold text-secondary border border-secondary/20 mb-2 inline-block">{(tournament?.sport_type ?? "MATCH").toString().toUpperCase()} • {(tournament?.category ?? "").toString().toUpperCase()}</span>
                    <h3 className="font-headline-md text-headline-md text-white">{match?.home_team_name ?? "TBD"} vs {match?.away_team_name ?? "TBD"}</h3>
                  </div>
                </div>
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex justify-between mb-4">
                    <div>
                      <p className="text-label-sm font-label-sm text-slate-500 mb-1">SCORE</p>
                      <p className="font-stats-numeric text-stats-numeric text-on-surface">{match?.home_score ?? 0} <span className="text-xs font-normal text-slate-500">- {match?.away_score ?? 0}</span></p>
                    </div>
                    <div className="text-right">
                      <p className="text-label-sm font-label-sm text-slate-500 mb-1">STATUS</p>
                      <p className="font-stats-numeric text-stats-numeric text-on-surface">{match?.status ?? "Scheduled"}</p>
                    </div>
                  </div>
                  <div className="w-full bg-slate-800 h-1.5 rounded-full mb-6 overflow-hidden">
                    <div className={`${isFinished ? "bg-slate-600" : "bg-secondary"} h-full rounded-full`} style={{ width: isFinished ? "100%" : isLive ? "50%" : "0%", boxShadow: isLive ? "0 0 10px rgba(74, 225, 118, 0.5)" : undefined }}></div>
                  </div>
                  <div className="mt-auto flex gap-3">
                    <button className="flex-1 bg-secondary text-slate-950 font-bold py-2.5 rounded-lg border-b-2 border-on-secondary-fixed-variant hover:brightness-110 active:translate-y-0.5 transition-all">Manage Match</button>
                    <button className="w-12 h-10 border border-slate-700 text-slate-400 flex items-center justify-center rounded-lg hover:border-secondary hover:text-secondary transition-colors">
                      <span className="material-symbols-outlined" data-icon="analytics">analytics</span>
                    </button>
                  </div>
        ...
        {/* Recent Activity Log (High Density Info Section) -> teams roster */}
        <div className="mt-12 bg-surface-container-low rounded-xl border border-outline-variant/30 overflow-hidden">
          <div className="px-6 py-4 border-b border-outline-variant/30 flex justify-between items-center">
            <h4 className="font-headline-md text-sm uppercase tracking-wider text-on-surface">Registered Teams</h4>
            <button className="text-secondary text-xs font-bold hover:underline">View All</button>
          </div>
          <div className="divide-y divide-outline-variant/10">
            {teams.map((team: any, idx: number) => (
              <div key={team?.id ?? idx} className="px-6 py-3 flex items-center gap-4 hover:bg-surface-container-high transition-colors">
                <span className="material-symbols-outlined text-secondary text-lg" data-icon="groups">groups</span>
                <span className="text-sm font-bold text-on-surface">{team?.name ?? "Unnamed Team"}</span>
                <span className="text-sm text-on-surface-variant flex-1">{team?.group_name ? `Group ${team.group_name}` : ""}</span>
                <span className="text-xs text-slate-500">{team?.status ?? ""}</span>
              </div>
            ))}
          </div>
        </div>
