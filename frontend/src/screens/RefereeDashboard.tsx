export default function RefereeDashboard({ data }: { data?: any }) {
  ...
  const tournamentName = data.tournament?.name ?? "Champion Hive";
  ...
  return (
    <div className="bg-background text-on-background font-body-md overflow-x-hidden">
      ...
      <main className="md:ml-64 pt-24 pb-24 px-md md:px-lg min-h-screen">
        <section className="mb-lg flex flex-col md:flex-row md:items-end justify-between gap-md">
          <div>
            <h1 className="font-display-xl text-display-xl text-on-surface">{tournamentName}</h1>
            <p className="text-on-surface-variant max-w-2xl mt-sm">Comprehensive performance analysis and activity log for senior official Marcus Thorne. Data updated in real-time based on league officiating standards.</p>
          </div>
          <div className="flex gap-sm">
            <div className="px-md py-sm bg-surface-container rounded-lg border border-outline-variant flex items-center gap-xs">
        ...
        <section className="grid grid-cols-1 md:grid-cols-4 gap-md mb-lg">
          <div className="stat-card-gradient p-lg rounded-xl border border-outline-variant border-accent-green flex flex-col justify-between h-40">
            ...
            <div>
              <h2 className="text-display-xl font-stats-numeric text-on-surface">{stats.matches ?? matches.length}</h2>
              <p className="text-secondary font-label-sm text-label-sm">{stats.finished_matches ?? 0} finished</p>
            </div>
          </div>
          <div className="stat-card-gradient p-lg rounded-xl border border-outline-variant border-accent-orange flex flex-col justify-between h-40">
            <div className="flex justify-between items-start">
              <span className="text-on-surface-variant font-label-sm text-label-sm">Teams</span>
              <span className="material-symbols-outlined text-tertiary">style</span>
            </div>
            <div>
              <h2 className="text-display-xl font-stats-numeric text-on-surface">{stats.teams ?? 0}</h2>
              <p className="text-on-surface-variant font-label-sm text-label-sm">Registered</p>
            </div>
          </div>
          <div className="stat-card-gradient p-lg rounded-xl border border-outline-variant border-accent-green flex flex-col justify-between h-40">
            <div className="flex justify-between items-start">
              <span className="text-on-surface-variant font-label-sm text-label-sm">Stages</span>
              <span className="material-symbols-outlined text-secondary">target</span>
            </div>
            <div>
              <h2 className="text-display-xl font-stats-numeric text-on-surface">{stats.stages ?? 0}</h2>
              <p className="text-secondary font-label-sm text-label-sm">Active</p>
            </div>
          </div>
          <div className="stat-card-gradient p-lg rounded-xl border border-outline-variant border-accent-green flex flex-col justify-between h-40">
            <div className="flex justify-between items-start">
              <span className="text-on-surface-variant font-label-sm text-label-sm">Finished Matches</span>
              <span className="material-symbols-outlined text-secondary">star</span>
            </div>
            <div>
              <h2 className="text-display-xl font-stats-numeric text-on-surface">{stats.finished_matches ?? 0}</h2>
              <p className="text-on-surface-variant font-label-sm text-label-sm">Completed</p>
            </div>
          </div>
        </section>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-lg">
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-surface-container-low text-on-surface-variant font-label-sm text-label-sm">
                    ...
                    <th className="p-md uppercase tracking-wider">Matchup</th>
                    <th className="p-md uppercase tracking-wider">Status</th>
                    <th className="p-md uppercase tracking-wider text-right">Score</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-outline-variant">
                  {matches.map((match: any) => (
                  <tr key={match?.id} className="hover:bg-surface-container-high transition-colors">
                    <td className="p-md font-stats-numeric text-on-surface">{match?.scheduled_start ? new Date(match.scheduled_start).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }) : '—'}</td>
                    <td className="p-md">
                      <div className="flex items-center gap-sm">
                        <div className="w-8 h-8 rounded bg-surface-container-highest flex items-center justify-center font-bold text-xs text-secondary">{(match?.home_team_name ?? '?').slice(0, 2).toUpperCase()}</div>
                        <div>
                          <p className="text-on-surface font-semibold">{match?.home_team_name ?? 'TBD'} vs {match?.away_team_name ?? 'TBD'}</p>
                          <p className="text-on-surface-variant text-xs">{match?.status ?? ''}</p>
                        </div>
                      </div>
                    </td>
                    <td className="p-md">
                    ...
                    <td className="p-md text-right font-stats-numeric text-secondary">{match?.home_score ?? 0} - {match?.away_score ?? 0}</td>
                  </tr>
                  ))}
                </tbody>
              </table>
