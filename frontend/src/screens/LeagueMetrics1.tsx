export default function LeagueMetrics1({ data }: { data?: any }) {
  const rows: any[] = data?.standingsFlat ?? [];
  const stats = data?.stats ?? null;
  const tournamentName = data?.tournament?.name;
  const leader = rows[0] ?? null;
  const formClasses = ["win", "win", "draw", "win", "loss"];
              ...
              <table className="w-full text-left">
                ...
                <tbody className="font-stats-numeric">
                  <tr className="group hover:bg-slate-800/30 transition-colors">
                    <td className="py-4 px-2 text-secondary font-bold">01</td>
                  ...
                  <tr className="group hover:bg-slate-800/30 transition-colors border-t border-slate-800/50">
                    <td className="py-4 px-2 text-on-surface font-bold">02</td>
                    <td className="py-4 px-2">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded bg-slate-800 border border-slate-700 flex items-center justify-center">
                  ...
                  <tr className="group hover:bg-slate-800/30 transition-colors border-t border-slate-800/50">
                    <td className="py-4 px-2 text-on-surface font-bold">03</td>
                    <td className="py-4 px-2">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded bg-slate-800 border border-slate-700 flex items-center justify-center">
                  ...
                  <tr className="group hover:bg-slate-800/30 transition-colors border-t border-slate-800/50">
                    <td className="py-4 px-2 text-on-surface font-bold">04</td>
                    <td className="py-4 px-2">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded bg-slate-800 border border-slate-700 flex items-center justify-center">
    ...
    <body className="bg-background text-on-surface font-body-md overflow-x-hidden">
      ...
      <main className="ml-64 mt-16 p-8 min-h-screen">
        ...
        <div className="flex justify-between items-end mb-8">
          <div>
            <nav className="flex items-center gap-2 text-xs text-slate-500 font-label-sm mb-2">
              <span>DASHBOARD</span>
              <span className="material-symbols-outlined text-[10px]">chevron_right</span>
              <span className="text-secondary">{tournamentName ? tournamentName.toUpperCase() : "PRO LEAGUE STANDINGS"}</span>
            </nav>
            <h2 className="font-headline-lg text-display-xl text-on-surface tracking-tighter">League Metrics</h2>
          </div>
          <div className="flex gap-3">
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-8 bg-surface-container-low rounded-xl border border-slate-800 p-6">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-headline-md text-on-surface">{tournamentName ? `${tournamentName} Standings` : "Pro Division Standings"}</h3>
              <div className="flex gap-2">
                <span className="bg-secondary/10 text-secondary text-[10px] font-bold px-2 py-1 rounded">LIVE UPDATING</span>
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="font-stats-numeric">
                  ...
                  <tr key={row?.team_id ?? i} className="group hover:bg-slate-800/30 transition-colors border-t border-slate-800/50 first:border-t-0">
                    <td className={`py-4 px-2 font-bold ${i === 0 ? "text-secondary" : "text-on-surface"}`}>{String(row?.position ?? i + 1).padStart(2, "0")}</td>
                    <td className="py-4 px-2">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded bg-slate-800 border border-slate-700 flex items-center justify-center">
                          <span className="material-symbols-outlined text-secondary" style={{ fontVariationSettings: "'FILL' 1" }}>bolt</span>
                    ...
                    <td className="py-4 px-2">
                      <div className="flex gap-1">
                        {formClasses.map((f, j) => (
                          <div key={j} className={`form-dot ${f}`}></div>
                        ))}
                      </div>
                    </td>
                  </tr>
            ...
            <div className="mt-6 pt-6 border-t border-slate-800 flex justify-center">
              <button className="text-secondary font-label-sm hover:underline">VIEW FULL STANDINGS ({stats?.teams ?? rows.length} TEAMS)</button>
            </div>
          </div>
          {/* Performance Curve (Visual Data) */}
          <div className="col-span-4 space-y-6">
            ...
            <div className="bg-surface-container-high rounded-xl border-l-4 border-secondary p-6 shadow-xl">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <p className="text-[10px] text-secondary font-bold uppercase tracking-widest mb-1">STREAK ALERT</p>
                  <h4 className="font-headline-md text-on-surface">{leader?.team_name ?? "Titan Velocity"}</h4>
                </div>
                <span className="material-symbols-outlined text-secondary text-3xl">trending_up</span>
              </div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-xs font-bold text-on-surface">{leader?.wins ?? 4} MATCH WINS</span>
              </div>
              <div className="text-[10px] text-slate-400 grid grid-cols-2 gap-2">
                <div className="bg-slate-950/50 p-2 rounded">
                  <p>SCORED</p>
                  <p className="text-on-surface font-bold text-sm">{leader?.points_scored ?? "92.4"}</p>
                </div>
                <div className="bg-slate-950/50 p-2 rounded">
                  <p>DIFF</p>
                  <p className="text-on-surface font-bold text-sm">{leader?.diff ?? "88.1"}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="col-span-12 grid grid-cols-4 gap-6">
            ...
            {/* Runner Ups */}
            <div className="col-span-1 flex flex-col gap-4">
              {(rows.length > 1 ? rows.slice(1, 4) : []).map((row: any, i: number) => (
              <div key={row?.team_id ?? i} className="bg-surface-container rounded-xl border border-slate-800 p-4 flex items-center gap-4 hover:border-slate-600 transition-all cursor-pointer">
                <div className="w-12 h-12 rounded bg-slate-800 flex-shrink-0 flex items-center justify-center font-bold text-slate-500">{row?.position ?? i + 2}</div>
                <div>
                  <p className="font-headline-md text-sm text-on-surface">{row?.team_name ?? "—"}</p>
                  <p className="text-xs text-slate-500">{row?.points_scored ?? 0} PTS SCORED</p>
                </div>
                <span className="material-symbols-outlined ml-auto text-slate-600">chevron_right</span>
              </div>
              ))}
